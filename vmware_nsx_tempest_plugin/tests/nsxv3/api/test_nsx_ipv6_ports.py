...
class IPv6PortsTest(feature_manager.FeatureManager):
    ...
    def _update_port_with_security_groups(self, security_groups_names):
        ...
        allocation_pools = {'allocation_pools': [{'start': str(address_cidr).split('/')[0]+'2',
                                                  'end':str(address_cidr).split('/')[0]+'70' }]}
        subnet_client = self.cmgr_adm.subnets_client
        subnet_name = network['name'] + 'sub'
        subnet_1 = self.create_topology_subnet(subnet_name, network,subnets_client=subnet_client,
                                             ip_version=6, enable_dhcp=False, **allocation_pools)
    ...
    def _create_ipv6_topology(self):
        ...
        subnet_client = self.cmgr_adm.subnets_client
        subnet_name = network['name'] + 'sub'
        subnet = self.create_topology_subnet(subnet_name, network,subnets_client=subnet_client,
                                             ip_version=6, enable_dhcp=False, **allocation_pools)
        return network
