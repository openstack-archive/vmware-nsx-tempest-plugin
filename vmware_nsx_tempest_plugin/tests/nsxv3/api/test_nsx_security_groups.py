...
class NSXv3SecGroupTest(base.BaseSecGroupTest):
    ...
    def _wait_till_firewall_gets_realize(self, secgroup, dfw_error_msg="", tenant_id=None):
        ...
        while nsx_firewall_time_counter < \
            ...
            if CONF.plugin.nsxp:
                nsx_nsgroup = self.nsxp.get_ns_group(secgroup['name'],
                                                secgroup['id'],os_tenant_id=tenant_id)
            else:
                nsx_nsgroup = self.nsx.get_ns_group(secgroup['name'],
                                                secgroup['id'])
            if CONF.plugin.nsxp:
                nsx_dfw_section = self.nsxp.get_firewall_section(secgroup['name'],
                                                            secgroup['id'],os_tenant_id=tenant_id)
            else:
    ...
    def test_create_sec_group_with_0_0_0_0_remote_ip_prefix(self):
        ...
        if CONF.plugin.nsxp:
            ...
            self._wait_till_firewall_gets_realize(secgroup, dfw_error_msg, secgroup['tenant_id'])
        else:
            nsx_nsgroup, nsx_dfw_section = \
    ...
    def test_create_update_nsx_security_group(self):
        ...
        if CONF.plugin.nsxp:
            self._wait_till_firewall_gets_realize(updated_secgroup, dfw_error_msg, tenant_id=updated_secgroup['tenant_id'])
        else:
    ...
    def test_delete_nsx_security_group(self):
        ...
        dfw_error_msg = "Firewall section not found for %s!" % secgroup['name']
        if CONF.plugin.nsxp :
            self._wait_till_firewall_gets_realize(secgroup, dfw_error_msg,tenant_id=secgroup['tenant_id'])
        else:
            self._wait_till_firewall_gets_realize(secgroup, dfw_error_msg)
        ...
        if CONF.plugin.nsxp:
            nsx_nsgroup = self.nsxp.get_ns_group(secgroup['name'],
                                            secgroup['id'], os_tenant_id=secgroup['tenant_id'])
        else:
            nsx_nsgroup = self.nsx.get_ns_group(secgroup['name'],
                                            secgroup['id'])
        if CONF.plugin.nsxp:
            nsx_dfw_section = self.nsxp.get_firewall_section(name, secgroup['id'],os_tenant_id=secgroup['tenant_id'])
        else:
    ...
    def test_create_nsx_security_group_rule(self):
        ...
        if CONF.plugin.nsxp:
            ...
            secgroup, dfw_error_msg, tenant_id=secgroup['tenant_id'])
        else:
            nsx_nsgroup, nsx_dfw_section = self._wait_till_firewall_gets_realize(
