...
class OctaviaRoundRobin(feature_manager.FeatureManager):
    ...
    def deploy_octavia_topology(self, no_of_servers=2, image_id=None):
        #router_lbaas = self.routers_admin_client.create_topology_router("router_lbaas")
        kwargs = {'name': "router_lbaas",
                  'external_gateway_info':
                  {"network_id": CONF.network.public_network_id}}
        router_lbaas = self.cmgr_adm.routers_client.create_router(**kwargs)
        self.addCleanup(test_utils.call_and_ignore_notfound_exc,self.routers_client.delete_router, router_lbaas['router']['id'])
        networks_client = self.cmgr_adm.networks_client
        name = "network_lbaas_1"
        network_lbaas_1 = self.create_topology_network(name,networks_client=networks_client)
        ...
        self.cmgr_adm.routers_client.add_router_interface(router_lbaas['router']['id'],
