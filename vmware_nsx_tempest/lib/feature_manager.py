...
class FeatureManager(traffic_manager.IperfManager):
    ...
    def check_downlink_port_created(self, router_op, subnet):
        tag_hit = 0
        backend_rtr_id = ''
        ...
        if backend_rtr_id:
            ...
            for ports in logical_rtr_ports:
                for rtr_tag in ports.get('tags'):
                    if rtr_tag['scope'] == "os-neutron-rport-id":
                        rtr_tag['tag'] = router['id']
                        tag_hit += 1
                    if rtr_tag['scope'] == "os-subnet-id":
                        subnet['id'] == rtr_tag['tag']
                        tag_hit += 1
                    if tag_hit == 2:
                        if ports.get('resource_type') \
                                == 'LogicalRouterDownLinkPort':
                            ip_address = ports.get('subnets')[
                                0].get('ip_addresses')
                            if ip_address:
                                if not self.\
    ...
    def check_centralized_port_created(self, router_op, subnet):
        tag_hit = 0
        backend_rtr_id = ''
        all_routers = self.nsx.get_logical_routers()
        for router in all_routers:
