...
class FeatureManager(traffic_manager.IperfManager,
    ...
    def create_zone(self, user=None, name=None, email=None, description=None,
        ...
        if user == None or user == 'admin':
            credentials = ['admin']
            _, body = self.admin_zones_client.create_zone(name=name, email=email,
                                                          description=description, wait_until=wait_until)
            self.addCleanup(test_utils.call_and_ignore_notfound_exc,
                            self.admin_zones_client.delete_zone, body['id'])
        elif user == 'primary':
            _, body = self.primary_zones_client.create_zone(name=name, email=email,
                                                          description=description, wait_until=wait_until)
            self.addCleanup(test_utils.call_and_ignore_notfound_exc,
