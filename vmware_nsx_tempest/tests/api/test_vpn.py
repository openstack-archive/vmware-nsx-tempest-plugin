...
class TestVpnOps(feature_manager.FeatureManager):
    ...
    def create_network_topo(self, enable_snat="False", cidr=None):
        kwargs = {}
        network = \
            self.create_topology_network(network_name="vpn-network")
        router_name = 'vpn-router'
        # Create router topo
        kwargs["enable_snat"] = enable_snat
        router = self.create_topology_router(router_name, **kwargs)
    ...
    def test_create_vpnservice_with_router_dhcp_disable(self):
