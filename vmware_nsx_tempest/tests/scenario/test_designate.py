...
class TestZonesV2Ops(feature_manager.FeatureManager):
    ...
    def create_zone_provider_vxlan_topology(self, zone_name):
        ...
        router = self.create_topology_router("router-designate")
        subnet_name = network_designate['name'] + '_subnet'
    ...
    def verify_ptr_recordset(self, recordset, count):
        self.assertEqual(record_set[1]['metadata']['total_count'], count)
        if any(record['type'] == 'NS'
               for record in record_set[1]['recordsets']):
            LOG.info('NS record is present')
        ...
        if count == 3:
            if any(record['type'] == 'PTR'
                   for record in record_set[1]['recordsets']):
                LOG.info('PTR record is present')
            else:
                LOG.error('PTR record is missing')
                raise Exception('ERROR: PTR record is absent')
    ...
    def verify_recordset_dns_name(self, recordset, dns_name):
        for record in record_set[1]['recordsets']:
            if record['type'] == 'PTR':
                if record['records'][0] == dns_name:
                    LOG.info('Record contains dns name of the vm')
                    return record
        return None
...
class TestZonesScenario(TestZonesV2Ops):
    ...
    @decorators.idempotent_id('a4de3cca-54e1-4e8b-8b52-2148e55eed84')
    def test_ptr_zone_list_with_fip_instance(self):
        """
        Create a zone, check zone exits
        Create a network and subnet
        Update network with the zone
        ...
        if record is None:
            raise Exception('fip is missing in the recordset')
        LOG.info('Show PTR records')
        fip_list = fip.split('.')
        rev_fip = fip_list[3] + '.' + fip_list[2] + '.' + fip_list[1] + '.' + fip_list[0]
        ptr_reverse_name = rev_fip + ".in-addr.arpa."
        LOG.info('Create PTR Zone')
        ptr_zone = self.create_zone(user = 'primary', name=ptr_reverse_name, email=zone['email'])
        #create ptr recordset
        ptr_recordset = self.create_recordset(record_type='PTR',
                                          zone_id = ptr_zone['id'],
                                          zone_name = ptr_reverse_name,
                                          record = ptr_record
                                          )
        ptr_record_set = self.create_ptr_recordset(ptr_reverse_name, ptr_zone['name'], ptr_zone['id'])
    ...
    def test_zone_list_with_fip_deletion_instance(self):
        ...
        fip_list = fip.split('.')
        rev_fip = fip_list[3] + '.' + fip_list[2] + '.' + fip_list[1] + '.' + fip_list[0]
        ptr_name = rev_fip + '.in-addr.arpa.'
        ptr_recordset = self.list_ptr_recordset(ptr_name)
        self.verify_ptr_recordset(recordset, 3)
        self.verify_recordset_dns_name(recordset, dns_name)
