...
class ProviderNetworks(base.BaseAdminNetworkTest):
    ...
    def resource_setup(cls):
        ...
        for tz in out:
            if "transport_type" in tz.keys():
                if tz['transport_type'] == "OVERLAY":
                    cls.overlay_id = tz['id']
                elif tz['transport_type'] == "VLAN":
                    cls.vlan_id = tz['id']
    ...
    def _create_provider_network(self, tz_id, net_type,
        ...
        if net_type == constants.VXLAN_TYPE:
            body = {"name": "provider-network-vxlan",
                    "provider:physical_network": tz_id,
                    "provider:network_type": net_type,
                    "admin_state_up": admin_state_up}
        elif net_type == constants.VLAN_TYPE:
            body = {"name": "provider-network-vlan",
                    "provider:segmentation_id": constants.VLAN,
                    "provider:network_type": net_type,
                    "provider:physical_network": tz_id,
                    "admin_state_up": admin_state_up}
    ...
    def test_provider_vlan_network(self):
        network = self._create_provider_network(self.vlan_id,
                                                constants.VLAN_TYPE)
        body = {"network_id": network['network']['id'],
                "allocation_pools": [{"start": "2.0.0.2",
                                      "end": "2.0.0.254"}],
