...
class TopoDeployScenarioManager(manager.NetworkScenarioTest):
    ...
    def create_server(self, name=None, image=None, flavor=None,
        ...
        if type(tenant_id) in (str, six.text_type):
    ...
    def waitfor_host_connected(self, host_ip, ping_timeout=5, msg=None):
        ...
        if msg and type(msg) in (str, six.text_type):
...
def check_hosts_connectivity(host, dest_list, ignore_helper=None,
    ...
    for dest in dest_list:
        ...
        if ('helper' in dest and type(ignore_helper) in (str, six.text_type)
            and re.search(ignore_helper, dest['helper'], re.I)):
            dest['reachable'] = None
            continue
